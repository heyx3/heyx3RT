	...
	public class RTShape_Mesh : RTShape
	{
		...
		protected override void ReadData(Serialization.DataReader reader)
		{
			...
			if (myMeshGUID == null || AssetDatabase.GUIDToAssetPath(myMeshGUID) == null)
			{
				...
				List<Vertex> verts = reader.List<Vertex>("Vertices",
					(Serialization.DataReader rd, ref Vertex vert, string name) => rd.Structure(vert, name));

				//Convert the read-in vertices to mesh data.
